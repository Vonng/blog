...
## 29. regresql —— 与语言无关的 SQL 回归测试工具

regresql 由 boringSQL（Radim Marek）开发，是一个**独立的 CLI 工具**（Go 编写），不是 PostgreSQL 扩展。它扫描项目中的 `*.sql` 文件，运行它们并保存输出快照与 EXPLAIN 计划基线，后续运行时对比差异，在 CI 中捕获查询结果变化与性能回退。支持 JUnit/GitHub Actions/pgTAP 输出格式。
...
工作流围绕 `discover`/`add`/`update`/`test`/`baseline`/`snapshot` 等命令组织。适合围绕 SQL 的快照测试（迁移/重构后快速发现结果差异）、EXPLAIN 基线（在 CI 里抓 plan flip 或 seq scan 退化）、把"SQL 真正确认过的输出"版本化。

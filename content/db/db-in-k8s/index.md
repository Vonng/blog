...
我们将自己的数据库服务架构方案开源出来 —— 即开箱即用的 PostgreSQL 发行版与本地优先的 RDS 替代： [Pigsty](https://pigsty.cc/docs/)。我们没有选择 K8S 与 Docker 这种所谓 “***一次构建，到处运行*** ”  的讨巧办法，而是一个一个地去适配不同的[操作系统发行版](https://mp.weixin.qq.com/s/xHG8OURTYlmnQTorFkzioA)/不同的大版本，并使用 Ansible 实现类 K8S CRD IaC 的效果封装管理复杂度。这确实是一件非常幸苦的工作，但却是正确的事情 —— 这个世界并不需要又一个在 K8S 中放入PG数据库玩具积木的拙劣尝试，但确实需要一个最大化发挥出硬件性能与可靠性的生产数据库服务架构方案。

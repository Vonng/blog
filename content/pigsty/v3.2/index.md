...
Pigsty v3.2 默认提供命令行工具 [**pig**](/pg/pig)，可以用来进一步简化 Pigsty 的安装部署配置过程。但 `pig` 并非仅仅是 Pigsty 的命令行工具，它还是一个可以独立使用的全功能 PostgreSQL 包管理器。
...
如果说 [**PostgreSQL 的未来是无法阻挡的可扩展性**](/pg/pg-eat-db-world)，Pig 就是帮助解锁这种能力的工具。毕竟，没有人会抱怨 PostgreSQL 实例拥有太多扩展 —— 不用的时候没有任何影响，需要时就在手边，随取随用。
...
Pig 的幕后支撑是一个充满稀缺扩展与新发布扩展的[**补充扩展仓库**](/pg/pg-ext-repo)，因此总可以轻松获取优质扩展 —— 经过测试、精心策划并准备就绪。

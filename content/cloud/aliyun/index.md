...
根因出在 **Auth** 上的可能性最大，最直接的证明就是：深度与 **Auth** 集成的云服务：对象存储 **OSS** （类 S3），表格存储 **OTS** （类 DynamoDB），以及其他深度依赖 **Auth** 的服务本身可用性直接受到影响。而本身运行不依赖 **Auth** 的云资源，比如云服务器 **ECS** / 云数据库 **RDS** 以及网络仍然可以“正常”运行，用户只是无法通过控制台与API对其进行管理与变更。此外，一个可以排除的**计量付费**服务出问题的现象是：故障期间还有用户成功付款[薅下了ECS的羊毛](/cloud/aliyun-ecs-pigsty/)。
...
马斯克的推特 X 和 DHH 的 37 Signal [通过下云省下了千万美元真金白银](/cloud/cloud-exit/)，创造了降本增效的“奇迹”，让下云开始成为一种潮流。云上的用户在对着账单犹豫着是否要下云，未上云的用户更是内心纠结。在这样的背景下，作为本土云领导者的阿里云发生如此重大故障，对于犹豫观望者的信心无疑是沉重的打击。恐怕此次故障会成为公有云**拐点级别的标志性事件**。
